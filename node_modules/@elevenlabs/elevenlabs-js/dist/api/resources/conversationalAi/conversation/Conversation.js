...
class Conversation extends events_1.EventEmitter {
    ...
    sendMultimodalMessage(options) {
        ...
        const fileIds = ((_a = options.fileIds) === null || _a === void 0 ? void 0 : _a.length) ? options.fileIds : options.fileId ? [options.fileId] : [];
        if (!options.text && fileIds.length === 0) {
            throw new Error("At least one of text, fileId, or fileIds must be provided");
        }
        const files = fileIds.map((file_id) => ({ type: "file_input", file_id }));
        const event = Object.assign(Object.assign({ type: events_2.ClientToOrchestratorEvent.MULTIMODAL_MESSAGE }, (options.text && { text: { type: events_2.ClientToOrchestratorEvent.USER_MESSAGE, text: options.text } })), (files.length && { file: files[0], files }));
        this.ws.send(JSON.stringify(event));
    }
    /**
